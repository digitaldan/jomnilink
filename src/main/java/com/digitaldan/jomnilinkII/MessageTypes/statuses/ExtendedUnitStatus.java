...
/*
 * EXTENDED UNIT STATUS (Requires Firmware Version 3.0 or Later)
 *
 ...
 *     Message length       (5 * number of units) + 3
 *     Message Type         0x3B
 *     Data 1               0x02 (object type)
 *     Data 2               0x05 (object record length)
 *     Data 3               unit number for first unit (MSB)
 *     Data 4               unit number for first unit (LSB)
 *     Data 5               unit status for first unit
 *     Data 6               high byte of time for first unit
 *     Data 7               low byte of time for first unit
 *     Data 8               unit number for second unit (MSB)
 *     Data 9               unit number for second unit (LSB)
 *     Data 10              unit status for second unit
 *     Data 11              high byte of time for second unit
 *     Data 12              low byte of time for second unit
 *     ...
...
public class ExtendedUnitStatus extends Status {
	@Delegate
	private final UnitStatus unitStatus;

	@Builder
	private ExtendedUnitStatus(int number, int status, int time) {
		super(number);
		unitStatus = UnitStatus.builder().number(number).status(status).time(time).build();
	}
}
