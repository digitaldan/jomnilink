 ...
 *     Message length       (6 * number of areas) + 3
 *     Message Type         0x3B
 *     Data 1               0x05 (object type)
 *     Data 2               0x06 (object record length)
 *     Data 3               area number for first area (MSB)
 *     Data 4               area number for first area (LSB)
 *     Data 5               area mode for first area
 *     Data 6               area alarms for first area
 *     Data 7               entry timer for first area
 *     Data 8               exit timer for first area
 *     Data 9               area number for second area (MSB)
 *     Data 10              area number for second area (LSB)
 *     Data 11              area mode for second area
 *     Data 12              area alarms for first area
 *     Data 13              entry timer for second area
 *     Data 14              exit timer for second area
 *     ...
...
public class ExtendedAreaStatus extends Status {
	@Delegate
	private final AreaStatus areaStatus;

	@Builder
	private ExtendedAreaStatus(int number, int mode, int alarms, int entryTimer, int exitTimer) {
		super(number);
		areaStatus = AreaStatus.builder().number(number).mode(mode).alarms(alarms).entryTimer(entryTimer)
				.exitTimer(exitTimer).build();
	}
}
